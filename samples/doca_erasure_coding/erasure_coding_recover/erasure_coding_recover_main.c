...
struct ec_cfg {
	...
	enum doca_ec_matrix_types matrix;		/* ec matrix type */
...
matrix_callback(void *param, void *config)
{
	...
	if (strcasecmp(matrix, "cauchy") == 0)
		ec_cfg->matrix = DOCA_CODING_MATRIX_CAUCHY;
	else if (strcasecmp(matrix, "vandermonde") == 0)
		ec_cfg->matrix = DOCA_CODING_MATRIX_VANDERMONDE;
	else {
...
/*
 * ARGP Callback - Handle redundecy block count parameter
 *
 * @param [in]: Input parameter
 * @config [in/out]: Program configuration context
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
...
register_ec_params(void)
{
	...
	doca_argp_param_set_description(delete_block_indices_param,
					"Indices of data blocks to delete comma seperated i.e. 0,3,4 - default: 0");
...
main(int argc, char **argv)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("ec_recover() encountered an error: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
