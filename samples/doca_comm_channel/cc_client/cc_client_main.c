...
/* Sample's Logic */
int create_comm_channel_client(const char *server_name, const char *dev_pci_addr, const char *text);
...
main(int argc, char **argv)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("create_comm_channel_client() encountered an error: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
