...
DOCA_LOG_REGISTER(BINARY_TREE::MAIN);

/* Sample's Logic */
doca_error_t binary_tree(struct dpa_resources *resources);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_dpa_binary_tree", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Running sample */
	result = binary_tree(&resources);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("binary_tree() encountered an error: %s", doca_get_error_string(result));
		goto dpa_cleanup;
	}
