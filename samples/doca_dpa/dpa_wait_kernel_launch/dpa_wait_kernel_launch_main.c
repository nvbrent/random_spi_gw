...
DOCA_LOG_REGISTER(WAIT_KERNEL_LAUNCH::MAIN);

/* Sample's Logic */
doca_error_t wait_kernel_launch(struct dpa_resources *resources);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_dpa_wait_kernel_launch", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to Allocate DPA Resources: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}

	/* Running sample */
	result = wait_kernel_launch(&resources);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("wait_kernel_launch() encountered an error: %s", doca_get_error_string(result));
		goto dpa_cleanup;
	}
...
dpa_cleanup:
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to destroy DOCA DPA resources: %s", doca_get_error_string(result));
