...
DOCA_LOG_REGISTER(ENDPOINT_COPY::MAIN);

/* Sample's Logic */
doca_error_t endpoint_copy(struct dpa_resources *resources);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_dpa_endpoint_copy", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Running sample */
	result = endpoint_copy(&resources);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("endpoint_copy() encountered an error: %s", doca_get_error_string(result));
		goto dpa_cleanup;
	}
