...
DOCA_LOG_REGISTER(FLOW_ACL::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_acl", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_acl(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_acl() encountered an error: %s", doca_get_error_string(result));
