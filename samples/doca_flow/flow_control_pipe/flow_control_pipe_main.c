...
DOCA_LOG_REGISTER(FLOW_CONTROL_PIPE::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_control_pipe", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_control_pipe(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_control_pipe() encountered an error: %s", doca_get_error_string(result));
