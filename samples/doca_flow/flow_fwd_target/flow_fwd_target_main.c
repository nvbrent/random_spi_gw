...
DOCA_LOG_REGISTER(FLOW_FWD_TARGET::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_fwd_target", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_fwd_target(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_fwd_target() encountered an error: %s", doca_get_error_string(result));
