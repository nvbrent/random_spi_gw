...
/*
 * Create DOCA Flow pipe with 5 tuple and src mac address match, and copy src mac action
 *
 * @port [in]: port of the pipe
 ...
 */
static doca_error_t
create_copy_to_meta_pipe(struct doca_flow_port *port, struct doca_flow_pipe *match_meta_pipe,
			 struct doca_flow_pipe **pipe)
{
	...
	/* copy src mac to u32[0]-u32[1] */
	desc_array[0].type = DOCA_FLOW_ACTION_COPY;
	desc_array[0].copy.src.field_string = "outer.eth.src_mac";
	desc_array[0].copy.src.bit_offset = 0;
	desc_array[0].copy.dst.field_string = "meta.data";
	desc_array[0].copy.dst.bit_offset = META_U32_BIT_OFFSET(0);
	desc_array[0].copy.width = 32;

	desc_array[1].type = DOCA_FLOW_ACTION_COPY;
	desc_array[1].copy.src.field_string = "outer.eth.src_mac";
	desc_array[1].copy.src.bit_offset = 32;
	desc_array[1].copy.dst.field_string = "meta.data";
	desc_array[1].copy.dst.bit_offset = META_U32_BIT_OFFSET(1);
	desc_array[1].copy.width = 16;
 ...
 */
static doca_error_t
add_copy_to_meta_pipe_entry(struct doca_flow_pipe *pipe, struct entries_status *status)
{
	struct doca_flow_match match;
	struct doca_flow_actions actions;
	struct doca_flow_pipe_entry *entry;
 ...
 */
doca_error_t
flow_copy_to_meta(int nb_queues)
{
