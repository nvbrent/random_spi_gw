...
DOCA_LOG_REGISTER(FLOW_COPY_TO_META::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_copy_to_meta", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_copy_to_meta(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_copy_to_meta() encountered an error: %s", doca_get_error_string(result));
