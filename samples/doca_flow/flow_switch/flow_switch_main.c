...
/* Sample's Logic */
doca_error_t flow_switch(int nb_queues, int nb_ports);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_switch", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_switch(dpdk_config.port_config.nb_queues, dpdk_config.port_config.nb_ports);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_switch() encountered an error: %s", doca_get_error_string(result));
