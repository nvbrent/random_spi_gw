...
struct doca_flow_pipe_entry *entries[NB_ENTRIES];	/* array for storing created entries */
...
create_host_to_network_pipe(struct doca_flow_port *sw_port, struct doca_flow_pipe **pipe)
{
	...
	return doca_flow_pipe_create(&pipe_cfg, /*&fwd*/NULL, &fwd_miss, pipe);
}
...
create_switch_pipe(struct doca_flow_port *sw_port, int nb_ports, struct doca_flow_pipe **pipe)
{
	...
	match_mask.meta.port_meta = UINT32_MAX;
...
flow_switch_direction_info(int nb_queues, int nb_ports)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create network to host pipe: %s", doca_get_error_string(result));
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create host to network pipe: %s", doca_get_error_string(result));
		stop_doca_flow_ports(nb_ports, ports);
		doca_flow_destroy();
		return result;
	}
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create switch pipe: %s", doca_get_error_string(result));
		stop_doca_flow_ports(nb_ports, ports);
		doca_flow_destroy();
		return result;
	}
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add entries to switch pipe: %s", doca_get_error_string(result));
		stop_doca_flow_ports(nb_ports, ports);
		doca_flow_destroy();
		return result;
	}
