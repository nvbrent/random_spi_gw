 ...
 */
static doca_error_t
create_switch_pipe(struct doca_flow_pipe **pipe)
{
 ...
 */
static doca_error_t
add_switch_pipe_entries(struct doca_flow_pipe *pipe, struct entries_status *status)
{
	struct doca_flow_match match;
	struct doca_flow_fwd fwd;
...
flow_switch(int nb_queues, int nb_ports)
{
	...
	result = create_switch_pipe(&pipe);
	if (result != DOCA_SUCCESS) {
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add entries to the pipe: %s", doca_get_error_string(result));
		stop_doca_flow_ports(nb_ports, ports);
		doca_flow_destroy();
		return result;
	}
