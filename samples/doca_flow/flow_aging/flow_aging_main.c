...
DOCA_LOG_REGISTER(FLOW_AGING::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_aging", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_aging(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_aging() encountered an error: %s", doca_get_error_string(result));
