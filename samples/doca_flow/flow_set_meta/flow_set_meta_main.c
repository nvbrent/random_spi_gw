...
DOCA_LOG_REGISTER(FLOW_SET_META::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_set_meta", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_set_meta(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_set_meta() encountered an error: %s", doca_get_error_string(result));
