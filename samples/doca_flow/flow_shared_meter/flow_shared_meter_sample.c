...
/* Meter colors green, yellow, red use 2 bits mask */
...
/*
 * Create DOCA Flow control pipe with match on greed/red colors and their respective forwarding.
 *
 * @port [in]: port of the pipe
 * @fwd_on_green [in]: the forwarding in case there is a green color match
 * @fwd_on_red [in]: the forwarding in case there is a red color match
 * @color_pipe [out]: created pipe pointer
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t
create_color_matched_pipe(struct doca_flow_port *port,
			  const struct doca_flow_fwd *fwd_on_green,
			  const struct doca_flow_fwd *fwd_on_red,
			  struct doca_flow_pipe **color_pipe)
{
	doca_error_t result;

	struct doca_flow_pipe_cfg pipe_cfg;
	struct doca_flow_match red_match, red_match_mask;
	struct doca_flow_match green_match, green_match_mask;

	memset(&pipe_cfg, 0, sizeof(pipe_cfg));
	memset(&red_match, 0, sizeof(red_match));
	memset(&red_match_mask, 0, sizeof(red_match_mask));
	memset(&green_match, 0, sizeof(green_match));
	memset(&green_match_mask, 0, sizeof(green_match_mask));

	pipe_cfg.attr.name = "MTR_CTRL_PIPE";
	pipe_cfg.attr.type = DOCA_FLOW_PIPE_CONTROL;
	pipe_cfg.attr.is_root = false;
	pipe_cfg.port = port;

	result = doca_flow_pipe_create(&pipe_cfg, NULL, NULL, color_pipe);
	if (result != DOCA_SUCCESS)
		return result;

	red_match.meta.meter_color = DOCA_FLOW_METER_COLOR_RED;
	*(uint8_t *)&red_match_mask.meta.meter_color = METER_COLOR_MASK;
	green_match.meta.meter_color = DOCA_FLOW_METER_COLOR_GREEN;
	*(uint8_t *)&green_match_mask.meta.meter_color = METER_COLOR_MASK;

	result = doca_flow_pipe_control_add_entry(0, 0, *color_pipe, &red_match, &red_match_mask, NULL, NULL, NULL, NULL, fwd_on_red, NULL, NULL);
	if (result)
		return result;
	result = doca_flow_pipe_control_add_entry(0, 0, *color_pipe, &green_match, &green_match_mask, NULL, NULL, NULL, NULL, fwd_on_green, NULL, NULL);
	if (result)
		return result;
...
/*
 * Create DOCA Flow pipe with 5 tuple match and monitor on shared meter ID
 *
 * @port [in]: port of the pipe
 * @out_l4_type [in]: l4 type to match: UDP/TCP
 * @color_pipe [in]: next forwarded pipe to match on color
 * @pipe [out]: created pipe pointer
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t
create_shared_meter_mark_pipe(struct doca_flow_port *port, enum doca_flow_l4_type_ext out_l4_type,
			      struct doca_flow_pipe *color_pipe, struct doca_flow_pipe **pipe)
{
	...
	monitor.shared_meter_id = 0xffffffff;

	/* Forward to next pipe to match on marked meter color */
	fwd.type = DOCA_FLOW_FWD_PIPE;
	fwd.next_pipe = color_pipe;

	return doca_flow_pipe_create(&pipe_cfg, &fwd, NULL, pipe);
}

/*
 * Add DOCA Flow pipe entry to the shared meter pipe
 *
 * @pipe [in]: pipe of the entry
 * @out_l4_type [in]: l4 type to match: UDP/TCP
 * @shared_meter_id [in]: ID of the shared meter
 * @status [in]: user context for adding entry
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t
add_shared_meter_pipe_entry(struct doca_flow_pipe *pipe, enum doca_flow_l4_type_ext out_l4_type,
			    uint32_t shared_meter_id, struct entries_status *status)
{
	struct doca_flow_match match;
	...
	/* set shared meter ID */
	monitor.shared_meter_id = shared_meter_id;
			 ...
			 struct doca_flow_pipe *udp_pipe, struct entries_status *status)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add control pipe entry: %s", doca_get_error_string(result));
		return result;
	}
...
/*
 * Run flow_shared_meter sample
 *
 * @nb_queues [in]: number of queues the sample will use
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
doca_error_t
flow_shared_meter(int nb_queues)
{
	int nb_ports = 2;
	...
	for (port_id = 0; port_id < nb_ports; port_id++) {
		...
		result = doca_flow_shared_resource_cfg(DOCA_FLOW_SHARED_RESOURCE_METER, shared_meter_ids[port_id],
						       &cfg);
		if (result != DOCA_SUCCESS) {
		...
		result = doca_flow_shared_resources_bind(DOCA_FLOW_SHARED_RESOURCE_METER, &shared_meter_ids[port_id], 1,
							 ports[port_id]);
		if (result != DOCA_SUCCESS) {
		...
		/* forwarding traffic to other port for passed (green) traffic */
		memset(&fwd_on_green, 0, sizeof(fwd_on_green));
		fwd_on_green.type = DOCA_FLOW_FWD_PORT;
		fwd_on_green.port_id = port_id ^ 1;
		/* drop on failed (red) traffic */
		memset(&fwd_on_red, 0, sizeof(fwd_on_red));
		fwd_on_red.type = DOCA_FLOW_FWD_DROP;

		result = create_color_matched_pipe(ports[port_id], &fwd_on_green, &fwd_on_red, &color_pipe);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to create color pipe: %s", doca_get_error_string(result));
			stop_doca_flow_ports(nb_ports, ports);
			doca_flow_destroy();
			return result;
		}

		result = create_shared_meter_mark_pipe(ports[port_id], DOCA_FLOW_L4_TYPE_EXT_TCP, color_pipe, &tcp_pipe);
		if (result < 0) {
		...
		result = add_shared_meter_pipe_entry(tcp_pipe, DOCA_FLOW_L4_TYPE_EXT_TCP, shared_meter_ids[port_id], &status);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to add entry: %s", doca_get_error_string(result));
			stop_doca_flow_ports(nb_ports, ports);
			doca_flow_destroy();
		...
		result = create_shared_meter_mark_pipe(ports[port_id], DOCA_FLOW_L4_TYPE_EXT_UDP, color_pipe, &udp_pipe);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to create pipe: %s", doca_get_error_string(result));
			stop_doca_flow_ports(nb_ports, ports);
			doca_flow_destroy();
		...
		result = add_shared_meter_pipe_entry(udp_pipe, DOCA_FLOW_L4_TYPE_EXT_UDP, shared_meter_ids[port_id], &status);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to add entry: %s", doca_get_error_string(result));
			stop_doca_flow_ports(nb_ports, ports);
			doca_flow_destroy();
