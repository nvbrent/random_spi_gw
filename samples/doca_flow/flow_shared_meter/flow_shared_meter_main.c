...
DOCA_LOG_REGISTER(FLOW_SHARED_METER::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_shared_meter", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_shared_meter(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_shared_meter() encountered an error: %s", doca_get_error_string(result));
