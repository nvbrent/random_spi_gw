...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_multi_fwd", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_multi_fwd(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_multi_fwd() encountered an error: %s", doca_get_error_string(result));
