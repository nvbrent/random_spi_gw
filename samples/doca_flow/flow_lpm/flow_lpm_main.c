...
DOCA_LOG_REGISTER(FLOW_LPM::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_lpm", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_lpm(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_lpm() encountered an error: %s", doca_get_error_string(result));
