...
DOCA_LOG_REGISTER(FLOW_MONITOR_METER::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_monitor_meter", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_monitor_meter(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_monitor_meter() encountered an error: %s", doca_get_error_string(result));
