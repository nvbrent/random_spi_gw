...
DOCA_LOG_REGISTER(FLOW_MULTI_ACTION::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_flow_multi_actions", NULL);
	if (result != DOCA_SUCCESS) {
	...
	/* run sample */
	result = flow_multi_actions(dpdk_config.port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("flow_multi_actions() encountered an error: %s", doca_get_error_string(result));
