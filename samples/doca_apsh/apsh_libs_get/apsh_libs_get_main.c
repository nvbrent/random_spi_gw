...
/* Sample's Logic */
doca_error_t libs_get(const char *dma_device_name, const char *pci_vuid, enum doca_apsh_system_os os_type,
...
main(int argc, char *argv[])
{
	...
	result = doca_argp_init("doca_apsh_libs_get", &apsh_conf);
	if (result != DOCA_SUCCESS) {
	...
	result = libs_get(apsh_conf.dma_dev_name, apsh_conf.system_vuid, apsh_conf.os_type, apsh_conf.pid);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("libs_get() encountered an error: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
