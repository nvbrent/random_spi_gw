...
DOCA_LOG_REGISTER(MODULES_GET::MAIN);
...
main(int argc, char *argv[])
{
	...
	result = doca_argp_init("doca_apsh_modules_get", &apsh_conf);
	if (result != DOCA_SUCCESS) {
	...
	result = modules_get(apsh_conf.dma_dev_name, apsh_conf.system_vuid, apsh_conf.os_type);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("modules_get() encountered an error: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
