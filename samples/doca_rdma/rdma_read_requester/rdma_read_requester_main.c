...
DOCA_LOG_REGISTER(RDMA_READ_REQUESTER::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_rdma_read_requester", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Start sample */
	result = rdma_read_requester(&cfg);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("rdma_read_requester() failed: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
