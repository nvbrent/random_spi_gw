...
write_read_connection(struct rdma_config *cfg, struct rdma_resources *resources)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to read the remote RDMA mmap connection details: %s", doca_get_error_string(result));
		return result;
	}
...
/*
 * Requester side of the RDMA read
 *
 * @cfg [in]: Configuration parameters
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
doca_error_t
rdma_read_requester(struct rdma_config *cfg)
{
	...
	void *dst_buf_data;
	struct doca_rdma_job_read_write job_read;
	...
	result = doca_rdma_connect(resources.rdma, resources.remote_rdma_conn_details,
					resources.remote_rdma_conn_details_size);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to connect the requester's DOCA RDMA to the responder's DOCA RDMA: %s", doca_get_error_string(result));
		goto destroy_resources;
	}
	...
	result = doca_buf_inventory_buf_by_data(buf_inventory, resources.remote_mmap, remote_mmap_range,
	...
	result = doca_buf_inventory_buf_by_addr(buf_inventory, resources.mmap, resources.mmap_memrange,
	...
	/* Set properties of DOCA RDMA READ job */
	job_read.base.ctx = resources.rdma_ctx;
	job_read.base.type = DOCA_RDMA_JOB_READ;
	job_read.base.flags = DOCA_JOB_FLAGS_NONE;
	job_read.src_buff = src_buf;
	job_read.dst_buff = dst_buf;
	job_read.base.user_data.u64 = 0;

	DOCA_LOG_INFO("Submitting DOCA RDMA read job that reads from the responder");
	/* Submit DOCA RDMA send job */
	result = doca_workq_submit(resources.workq, (struct doca_job *)(&job_read));
	if (result != DOCA_SUCCESS) {
	...
	DOCA_LOG_INFO("Retrieving results from the DOCA RDMA read job");
	...
	if (job_read.base.user_data.u64 != event.user_data.u64) {
	...
	/* Read the data that is on the dst_buf */
	...
	DOCA_LOG_INFO("Read from responder: \"%s\"\n", (char *)dst_buf_data);
