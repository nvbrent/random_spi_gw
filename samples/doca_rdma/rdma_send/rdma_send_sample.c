...
rdma_send(struct rdma_config *cfg)
{
	...
	/* Set properties of DOCA RDMA send job */
	job_send.base.ctx = resources.rdma_ctx;
	job_send.base.type = DOCA_RDMA_JOB_SEND;
	job_send.base.flags = DOCA_JOB_FLAGS_NONE;
	job_send.src_buff = src_buf;
	job_send.base.user_data.u64 = 0;

	DOCA_LOG_INFO("Submitting DOCA RDMA send job that sends \"%s\" to receiver", cfg->send_string);
	/* Submit DOCA RDMA send job */
	result = doca_workq_submit(resources.workq, (struct doca_job *)(&job_send));
	if (result != DOCA_SUCCESS) {
	...
	DOCA_LOG_INFO("Retrieving results from the DOCA RDMA send job");
	...
	if (job_send.base.user_data.u64 != event.user_data.u64) {
