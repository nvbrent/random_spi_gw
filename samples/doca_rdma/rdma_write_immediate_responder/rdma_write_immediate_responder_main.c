...
DOCA_LOG_REGISTER(RDMA_WRITE_IMMEDIATE_RESPONDER::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_rdma_write_immediate_responder", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Start sample */
	result = rdma_write_immediate_responder(&cfg);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("rdma_write_immediate_responder() failed: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
