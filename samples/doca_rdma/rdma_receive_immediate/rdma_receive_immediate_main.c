...
DOCA_LOG_REGISTER(RDMA_RECEIVE_IMMEDIATE::MAIN);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_rdma_receive_immediate", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Start sample */
	result = rdma_receive_immediate(&cfg);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("rdma_receive_immediate() failed: %s", doca_get_error_string(result));
		goto argp_cleanup;
	}
