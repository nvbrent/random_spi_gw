...
write_read_connection(struct rdma_config *cfg, struct rdma_resources *resources)
{
	...
	DOCA_LOG_INFO("Please copy %s and %s from the receiver and then press enter",
 ...
 */
doca_error_t
rdma_write_immediate_requester(struct rdma_config *cfg)
{
	...
	struct doca_rdma_job_read_write job_write_immediate;
	...
	/* Set properties of DOCA RDMA write with immediate job */
	job_write_immediate.base.ctx = resources.rdma_ctx;
	job_write_immediate.base.type = DOCA_RDMA_JOB_WRITE_IMM;
	job_write_immediate.base.flags = DOCA_JOB_FLAGS_NONE;
	job_write_immediate.src_buff = src_buf;
	job_write_immediate.dst_buff = dst_buf;
	job_write_immediate.immediate_data = EXAMPLE_IMMEDIATE_VALUE;
	job_write_immediate.base.user_data.u64 = 0;

	DOCA_LOG_INFO("Submitting DOCA RDMA write with immediate job that writes to the responder");
	/* Submit DOCA RDMA write with immediate job */
	result = doca_workq_submit(resources.workq, (struct doca_job *)(&job_write_immediate));
	if (result != DOCA_SUCCESS) {
	...
	DOCA_LOG_INFO("Retrieving results from the DOCA RDMA write with immediate job");
	...
	if (job_write_immediate.base.user_data.u64 != event.user_data.u64) {
	...
	DOCA_LOG_INFO("Written to responder \"%s\" with immediate data: %u\n", cfg->write_string,
			job_write_immediate.immediate_data);
destroy_dst_buf:
