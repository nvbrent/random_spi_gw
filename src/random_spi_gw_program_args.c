...
dmac_callback(void *param, void *config_voidp)
{
	...
	DOCA_LOG_INFO("Selected encap/outer dmac: %s", param_str);
	return DOCA_SUCCESS;
}

static doca_error_t
inner_dmac_callback(void *param, void *config_voidp)
{
	struct random_spi_gw_config * config = config_voidp;
	const char *param_str = param;

	rte_ether_unformat_addr(param_str, &config->decap_eth_hdr.dst_addr);
	DOCA_LOG_INFO("Selected decap/inner dmac: %s", param_str);
	return DOCA_SUCCESS;
}
...
random_spi_gw_register_argp_params(void)
{
	...
	doca_argp_param_set_long_name(param, "inner-dmac");
	doca_argp_param_set_description(param, "Decap destination mac addr");
	doca_argp_param_set_callback(param, inner_dmac_callback);
	doca_argp_param_set_mandatory(param);
	doca_argp_param_set_type(param, DOCA_ARGP_TYPE_STRING);
	ret = doca_argp_register_param(param);
	if (ret != DOCA_SUCCESS) {
