...
nat_init(struct nat_cfg *app_cfg, struct application_dpdk_config *app_dpdk_config)
{
	...
	nat_flow_cfg.queues = app_dpdk_config->port_config.nb_queues;
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to init doca flow ports: %s", doca_get_error_string(result));
		return result;
	}
...
build_static_local_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create nat pipe: %s", doca_get_error_string(result));
		return result;
	}
	...
	for (ruleid = 0; ruleid < nat_num_rules; ruleid++) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Entry creation FAILED: %s", doca_get_error_string(result));
			continue;
		}
		nb_entries++;
...
build_pat_local_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create NAT TCP pipe: %s", doca_get_error_string(result));
		return result;
	}
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create NAT UDP pipe: %s", doca_get_error_string(result));
		return result;
	}
	...
	result = add_control_pipe_local_entries(control_pipe, nat_udp_pipe, nat_tcp_pipe, ports[port_id]);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add control pipe entries: %s", doca_get_error_string(result));
		return result;
	}
...
build_pat_global_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	...
	result = add_control_pipe_global_entries(control_pipe, nat_udp_pipe, nat_tcp_pipe, ports[port_id]);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add control pipe entries: %s", doca_get_error_string(result));
		return result;
	}
