...
add_entry(uint16_t pipe_queue, uint32_t flag, struct flow_skeleton_entry *entry)
{
	...
	case DOCA_FLOW_PIPE_CONTROL:
		result = doca_flow_pipe_control_add_entry(pipe_queue, entry->ctx.priority, entry->ctx.pipe, entry->ctx.match, entry->ctx.match_mask,
...
add_batch_entries(struct main_loop_params *params, uint32_t nb_entries, int port_id)
{
	...
	for (i = 0; i < nb_entries - 1; i++) {
		...
		if (result != DOCA_SUCCESS)
			DOCA_LOG_ERR("Failed to add/remove entry in index [%d]: %s", i, doca_get_error_string(result));
		else
	...
	if (result != DOCA_SUCCESS)
		DOCA_LOG_ERR("Failed to add/remove entry in index [%d]: %s", nb_entries - 1, doca_get_error_string(result));
	else
		skeleton_ctx.queue_state[port_id][params->pipe_queue]++;

	result = doca_flow_entries_process(params->ports[port_id], params->pipe_queue, 0, nb_entries);
	if (result != DOCA_SUCCESS)
		DOCA_LOG_ERR("DOCA Flow entries process failed %s", doca_get_error_string(result));
