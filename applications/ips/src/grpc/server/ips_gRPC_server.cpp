 ...
 */
static doca_error_t
run_server(const char *arg, struct ips_config *ips_config)
{
	...
	while (!force_quit) {
		...
		if (ips_config->netflow_source_id && (result = send_netflow_record()) != DOCA_SUCCESS) {
...
/*
 * IPS gRPC application main function
 *
...
main(int argc, char *argv[])
{
	...
	ips_config.dpdk_config = &dpdk_config;
	...
	result = doca_argp_init("doca_ips_grpc", &ips_config);
	if (result != DOCA_SUCCESS) {
	...
	/* Start the server */
	result = run_server(grpc_address, &ips_config);
	if (result != DOCA_SUCCESS)
		exit_status = EXIT_FAILURE;
