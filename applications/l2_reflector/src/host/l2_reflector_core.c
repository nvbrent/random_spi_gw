...
l2_reflector_setup_ibv_device(struct l2_reflector_config *app_cfg)
{
	...
	app_cfg->uar = mlx5dv_devx_alloc_uar(app_cfg->ibv_ctx, MLX5DV_UAR_ALLOC_TYPE_NC);
	if (app_cfg->uar == NULL) {
	...
	if (app_cfg->pd == NULL) {
		...
		mlx5dv_devx_free_uar(app_cfg->uar);
		ibv_close_device(app_cfg->ibv_ctx);
		return DOCA_ERROR_DRIVER;
	}
	return DOCA_SUCCESS;
...
l2_reflector_setup_device(struct l2_reflector_config *app_cfg)
{
	...
	result = flexio_uar_create(app_cfg->flexio_process, app_cfg->uar, &app_cfg->flexio_uar);
	if (result != FLEXIO_STATUS_SUCCESS) {
		DOCA_LOG_ERR("Could not create FlexIO UAR (%d)", result);
		flexio_process_destroy(app_cfg->flexio_process);
		return DOCA_ERROR_DRIVER;
	}
...
l2_reflector_device_destroy(struct l2_reflector_config *app_cfg)
{
	...
	ret |= flexio_uar_destroy(app_cfg->flexio_uar);
...
device_callback(void *param, void *config)
{
	...
	app_cfg->device_name = device_name;
	return DOCA_SUCCESS;
}
