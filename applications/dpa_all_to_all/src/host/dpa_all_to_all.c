...
prepare_argp_parameters(int argc, char **argv, struct a2a_config *cfg)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to register application parameters: %s", doca_get_error_string(result));
		doca_argp_destroy();
		return result;
	}
...
main(int argc, char **argv)
{
	...
	if (result != DOCA_SUCCESS) {
		if (rank == 0)
			DOCA_LOG_ERR("dpa_a2a() encountered errors: %s", doca_get_error_string(result));
		exit_status = EXIT_FAILURE;
	}
