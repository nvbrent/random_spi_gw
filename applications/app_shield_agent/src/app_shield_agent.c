...
main(int argc, char **argv)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to init application: %s", doca_get_error_string(result));
		doca_argp_destroy();
		return EXIT_FAILURE;
	}
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Apsh init was successful but failed to read process %d information: %s", apsh_conf.pid, doca_get_error_string(result));
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Attestation init failed: %s", doca_get_error_string(result));
		exit_status = EXIT_FAILURE;
